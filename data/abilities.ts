...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	tikimask: {
		...
		onDamage(damage, target, source, effect) {
			if (
				effect && effect.effectType === 'Move' &&
				['tikape', 'goriki', 'tikikong'].includes(target.species.id) && !target.transformed
			) {
				this.add('-activate', target, 'ability: Tiki Mask');
				this.effectState.busted = true;
				this.boost({atk: 1, def: -1, spa: 1, spd: -1}, source);
				return 0;
			}
		},
		onCriticalHit(target, source, move) {
			if (!target) return;
			if (!['tikape', 'goriki', 'tikikong'].includes(target.species.id) || target.transformed) {
				return;
			}
			const hitSub = target.volatiles['substitute'] && !move.flags['bypasssub'] && !(move.infiltrates && this.gen >= 6);
			if (hitSub) return;
	...
	reptilian: {
		onWeather(target, source, effect) {
			if (target.hasItem('utilityumbrella')) return;
			if (effect.id === 'sunnday' || effect.id === 'desolateland') {
				this.heal(target.baseMaxhp / 8);
			} else if (effect.id === 'snowstorm') {
				this.damage(target.baseMaxhp / 8, target, target);
			}
		},
		isBreakable: true,
	...
	territorial: {
		onSwitchIn(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				if (!activated) {
					this.add('-ability', pokemon, 'Territorial', 'boost');
					activated = true;
	...
	radiate: {
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				if (!activated) {
					this.add('-ability', pokemon, 'Radiate', 'boost');
					activated = true;
				}
				if (target.volatiles['substitute']) {
					this.add('-immune', target);
	...
	stunningentrance: {
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				if (!activated) {
					this.add('-ability', pokemon, 'Stunning Entrance', 'boost');
					activated = true;
				}
				if (target.volatiles['substitute']) {
					this.add('-immune', target);
	...
	cropduster: {
		...
		onSourceModifyAtk(atk, pokemon, target, move) {
			if (pokemon.hasType('Grass') || pokemon.hasType('Flying')) {
				return this.chainModify(2);
			}
		},
		name: "Cropduster",
	...
	jellyarmor: {
		onTryHit(pokemon, target, move) {
			if (move.flags['bullet']) {
				...
				this.damage(pokemon.baseMaxhp / 12, pokemon, target);
				return null;
			}
		},
		isBreakable: true,
...
//	},
	nitrogenize: {
		// upokecenter says this is implemented as an added secondary effect
		onModifyMove(move) {
			if (move?.type === 'Ice' && move.target === 'self') return;
			if (!move.secondaries) {
				move.secondaries = [];
			}
			move.secondaries.push({
				chance: 30,
				status: 'brn',
				ability: this.dex.abilities.get('nitrogenize'),
			});
		},
		name: "Nitrogenize",
		rating: 2,
	...
	fromtheashes: {
		...
		onDamage(damage, target, source, effect) {
			if (target.hp === 0 && damage >= target.hp && effect && effect.effectType === 'Move') {
				this.add('-ability', target, 'From the ashes');
	...
	frailskin: {
		onDamagingHit(damage, target, source, move) {
			if (move.category === 'Physical') {
				this.boost({def: -1, spa: 2}, target, target);
			}
		},
		name: "Frail Skin",
		rating: 1,
