...
Impale: get stabbed but more lol
...
export const Moves: {[moveid: string]: MoveData} = {
	...
	bananabeat: {
		...
		flags: {snatch: 1},
		onHit(target) {
			if (target.hp <= target.maxhp / 1.3 || target.boosts.atk >= 4 || target.maxhp === 1) { // Shedinja clause
				return false;
			}
			this.directDamage(target.maxhp / 1.5);
	...
	dragondarts: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, impale: 1},
		multihit: 2,
		smartTarget: true,
		secondary: null,
		target: "normal",
	...
	drillpeck: {
		...
		pp: 20,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, distance: 1, impale: 1},
		secondary: null,
		target: "any",
		type: "Flying",
		contestType: "Cool",
	...
	stalagtites: {
		...
		priority: 0,
		flags: {nonsky: 1},
		sideCondition: 'stalagtites',
		condition: {
			onFieldStart() {
				this.add('-fieldstart', 'move: stalagtites');
				for (const pokemon of this.getAllActive()) {
					let applies = false;
					if (pokemon.removeVolatile('bounce') || pokemon.removeVolatile('fly')) {
						applies = true;
	...
	hornleech: {
		...
		pp: 10,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, heal: 1, impale: 1},
		drain: [1, 2],
		secondary: null,
		target: "normal",
		type: "Grass",
	...
	cruelsun: {
		...
		onTryMove(attacker, defender, move) {
			...
			this.add('-prepare', attacker, move.name);
			this.boost({def: -1}, attacker, defender, move);
			if (!this.runEvent('ChargeMove', attacker, defender, move)) {
				return;
			}
			attacker.addVolatile('twoturnmove', defender);
